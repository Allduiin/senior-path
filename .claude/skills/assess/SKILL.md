...
# /assess — periodic deep re-assessment  ·  STATUS: sketch v0

> **Sketch — refine after the first real run.** The selection and quiz mechanics below are sound;
> the **calibration rules** in step 4 (cell scores → pillar levels) are provisional and should be
> firmed up the first time this runs against real data, then this notice removed.
...
4. **Recalibrate (provisional rules — refine on first run):**
   ...
   - Recompute **pillar levels** in `progress-log.md` from that pillar's cell scores, e.g.
