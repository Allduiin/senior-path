...
## Objective
Build the reflex for the **lost-update anomaly**: see *why* a read-modify-write over a row is
not atomic under PostgreSQL's default `READ COMMITTED`, and fix it with the correct concurrency
control (optimistic version, pessimistic row lock, or a single atomic statement) rather than by
escalating the global isolation level or shrinking the window.

## Scenario
A `WalletService.withdraw(walletId, amount)` does the textbook three-step: **read** the balance,
...
The test fires **16** concurrent withdrawals of €1.00 at one wallet seeded with €1000.00, released
together by a barrier. Correct end-state: balance €984.00 (16 withdrawals applied). Buggy
end-state: balance is far higher — most withdrawals overwrite each other and are **lost**.

## Tasks
1. **Diagnose (Analysis section).** Explain the mechanism: how `READ COMMITTED` per-statement
   snapshots let two transactions both read balance `B`, both write `B - amount`, last-writer-wins.
   Contrast `REPEATABLE READ` and `SERIALIZABLE`, PostgreSQL MVCC/snapshot isolation vs InnoDB's
   `REPEATABLE READ`, and say where the lost update is stopped in each.
...
- Testcontainers Postgres only — no manual DB setup.
...
> _TODO: mechanism of the lost update; isolation-level comparison table; PG MVCC vs InnoDB RR;
> chosen fix and why; tradeoffs of optimistic vs pessimistic vs atomic._
