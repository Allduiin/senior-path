...
| 2026-08-26 | Spaced-review cold pass (`/repeat-knowledge`) — **evidence only, `Latest` deliberately not moved** | Q7, Q8 | Retention grades: **Q7 77**, **Q8 70**, both q3 passes. Recorded here because they materially contradict the `Latest` column (82 / 84, set from stage-5 review work that included code + Analysis): a cold quiz is a narrower instrument than a stage-5 gate, and the `Latest` column is `/assess`-owned, so blending the two would be exactly the drift the one-owner rule forbids. **`/assess` (overdue since 2026-08-18) owns the reconciliation.** Q7: recovered the crash-window asymmetry and the TTL principle, and named the PSP producer-sent `Idempotency-Key` as the first-call prerequisite — the design inversion open since 08-17, now closed; still mislabels broker requeue as "retry" (3rd session). Q8: Kafka EOS answered beyond the KB note (`InitProducerId` PID recovery, epoch fencing under `transactional.id`, and why a restarted relay's `send()` is a legitimately new record); missed the perfect-network process-death window (assumed away process failure, then reintroduced network loss) and left the publisher-confirm **direction** unanswered for the second session. Q6 and Q2 were due but not reviewed — session closed early. |
