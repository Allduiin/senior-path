...
## Next session focus
**Q8 is at stage 3 — scaffold p1-03 arc stage C** (`/next-exercise`, targeting the same module):
delivery semantics made observable in code. Candidate shape: expose the ack-placement axis (the
at-most-once trap) and/or the composition's failure when one ingredient is removed. Then stage 4
is the user's.
