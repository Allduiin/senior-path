...
## Next session focus
**Q8 is at stage 1 — teach delivery semantics** (`/learn-theme Q8`), the arc's last stage:
at-most/at-least/exactly-once and why the third is impossible on the wire, Two Generals, ack
ordering as the choice of which side the failure window falls on, effectively-once as
composition (at-least-once transport + idempotent effect), and where the arc's own pipeline
sits on that map. Then stage C of `p1-03-outbox-arc` against the same module.
